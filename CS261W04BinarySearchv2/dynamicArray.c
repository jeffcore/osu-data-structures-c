...
/* 	Adds to a position in array and moves all element up

	param: 	da		pointer to the dynamic array
	param:	index		the index position to add the new array
	param:	newElement		the value to add to the end of the dynamic array
	pre:	the index is within the size of the array
	post:	size increases by 1
	post:	if reached capacity, capacity is doubled
	post:	val is in the last utilized position in the array
*/
...
/* 	Binary Search for value in dynamic array

	param: 	data		pointer to the array in dynamic array
	param:	size		size of dynamic array
	param:	testValue		the value to search for or
	return: index position	of value found, or position of insertion
*/
...
/* 	Binary Search for value in dynamic array
	...
	param: 	dy		pointer to the dynamic array
	param:	testValue		the value to search for or
	return: index position	of value found, or position of insertion
*/
int dyArrayBinarySearch (struct DynArr * dy, TYPE testValue)
{
	return _binarySearch (dy->data, dy->size, testValue);
}


/* 	add item to dynamic array in order

	param: 	dy		pointer to the dynamic array
	param:	newElement		value to add
	post:	size increases by 1
	post:	if reached capacity, capacity is doubled
	post:	val in placed in sorted order
*/
...
/*  Check if item is contained in the dynmamic array

	param: 	dy		pointer to the dynamic array
	param:	testElement		value to search for
	return:   1 found 0 did not
*/
int orderedArrayContains (struct DynArr *dy, TYPE testElement)
{
	int index = dyArrayBinarySearch(dy, testElement);
	if (index < dy->size && dy->data[index] == testElement)
	{
		return (1);
	}
	else
...
*/
void orderedArrayRemove (struct DynArr *dy, TYPE testElement)
{
	int index = dyArrayBinarySearch(dy, testElement);
