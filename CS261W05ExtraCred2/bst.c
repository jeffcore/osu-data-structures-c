...
/*----------------------------------------------------------------------------*/
/*
 itereatvie helper function to add a node to the binary search tree.
 HINT: You have to use the compare() function to compare values.
 param:  cur	the current root node
		 val	the value to be added to the binary search tree
 pre:	val is not null
 */
struct Node * _addNode2(struct Node *cur, TYPE val)
{
	assert(val != 0);
	int gameState = 1;
	struct Node *saveNode = cur;
		...
		while (gameState)
		{
			...
			else
			{
				if (cur->right == 0)
				{
					...
					assert(newNode2 != 0);
					newNode2->val = val;
					newNode2->left = newNode2->right = 0;
...
void addBSTree2(struct BSTree *tree, TYPE val)
{
	assert(tree != 0);
	assert(val != 0);

	tree->root = _addNode2(tree->root, val);
	tree->cnt++;
}
...
void _removeNode2(struct Node *node, TYPE value	)
{
	struct Node *current = NULL, *del_node = NULL, *child = NULL;
	struct Node *parent = NULL, *x = NULL;
		current = node;

		while (current != NULL) {
			 if (compare(value, current->val)== 0) {
							del_node = current;
			...
			else if(compare(value, current->val) == -1)
			{
				parent = current;
				current = current->left;
			}
...
void removeBSTree2(struct BSTree *tree, TYPE val)
{
	assert(tree != 0);
	assert(val != NULL);
	if (containsBSTree(tree, val)) {
		_removeNode2(tree->root, val);
		tree->cnt--;
	}
}
...
void testRemoveNode() {
    ...
    _removeNode2(tree->root, &myData4);
    printTestResult(compare(tree->root->val, &myData1) == 0 && tree->root->left->left == 0, "_removeNode", "remove left of left of root 1st try");
	        
    _removeNode2(tree->root, &myData3);
	 printTestResult(compare(tree->root->val, &myData1) == 0 && tree->root->right == 0, "_removeNode", "remove right of root 2st try");
	   
    _removeNode2(tree->root, &myData2);
