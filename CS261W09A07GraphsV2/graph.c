...
int DFS(Graph* g, Vertex* source, Vertex* destination)
{
	...
	struct cirListDeque *reachable = malloc(sizeof(struct cirListDeque));
	assert(reachable != 0);
	initCirListDeque(reachable);
	struct cirListDeque *container = malloc(sizeof(struct cirListDeque));
	assert(container != 0);
	initCirListDeque(container);

	struct Vertex *tempV;

	addFrontCirListDeque(container, source);

	while(!(isEmptyCirListDeque(container)))
	{
		tempV = frontCirListDeque(container);
		removeFrontCirListDeque(container);
		if(!tempV->isVisited)
		{
			addFrontCirListDeque(reachable, tempV);

			tempV->isVisited = 1;

			if(tempV == destination)
				return 1;

			for(i = 0; i < tempV->numNeighbors; ++i)
			{
				if(!tempV->neighbors[i]->isVisited)
					addFrontCirListDeque(container,  tempV->neighbors[i]);
			}
		}
	}
...
int BFS(Graph* g, Vertex* source, Vertex* destination)
{
	...
	addBackCirListDeque(container, source);
	while(!(isEmptyCirListDeque(container)))
	{
		tempV = frontCirListDeque(container);
		removeFrontCirListDeque(container);
		if(!tempV->isVisited)
		{
			...
			{
				if(!tempV->neighbors[i]->isVisited)
					addBackCirListDeque(container,  tempV->neighbors[i]);
			}
		}
	}
