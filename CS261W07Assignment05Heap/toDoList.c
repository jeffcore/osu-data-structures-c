...
void printList(DynArr *heap)
{
	...
	assert(sizeDynArr(heap) > 0);
	/*sort heap before printing */

	struct DynArr *heapNew = malloc(sizeof(struct DynArr *));
	copyDynArr(heap, heapNew);
	sortHeap(heapNew);

	int i;
	Task* task;
	/*loop through dynamic array and print all values */
	for(i = sizeDynArr(heapNew) - 1; i >= 0; i--)
	{
		task = getDynArr(heapNew, i);
		print_type(task);
	}
...
{
	struct Task* leftNew =  (struct Task*)left;
	struct Task* rightNew = (struct Task*)right;


	if (leftNew->priority == rightNew->priority)
	{
		return 0;
	}
	else if (leftNew->priority < rightNew->priority)
	{
		return -1;
	}
	else
...
{
    /*FIXME: write this  -- looks fine to me*/
	Task* data1;
	data1 = (Task*)curval;
