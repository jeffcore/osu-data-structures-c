...
void addHeap(DynArr *heap, TYPE val)
{
	...
	int parent;
	int pos = sizeDynArr(heap);  /*gets last open index	*/
	addDynArr(heap, val);/*puts value in to next open spot*/

	/*percolate up part	*/
	while(pos!=0)
	{
		parent = (pos-1)/2;   /*get the parent of the index	*/
		if(compare(getDynArr(heap, pos), getDynArr(heap, parent)) == -1)  /*compare value of pos and parent, if pos is less swap	*/
		{
			swapDynArr(heap, pos, parent);
			pos = parent;	 /*reset current position to parent	*/
		}
		else
		{
			return;
...
void _adjustHeap(DynArr *heap, int max, int pos)
{
	...
	int leftChild = (2*pos+1);    /*get left child of node	*/
	int rightChild = (2*pos+2);   /*get right child of node	*/
	int smallestIndex;

	if(rightChild < max) /*has two children	*/
	{
		 /*compare left and right child to get smaller node	*/
		smallestIndex = _smallerIndexHeap(heap, leftChild, rightChild);
		 /*compare smaller node to parent, if child node is smaller swap and adjust heap*/
	...
	else if(leftChild < max) /*has only left child*/
	{
		 /*compare left node to parent, if left node is smaller swap and adjust heap*/
...
void _buildHeap(DynArr *heap)
{
	...
	for ( i = max/2-1; i >= 0; i--)
	{
		_adjustHeap(heap, max, i);  /* build proper heap order from inner nodes */
	}
}
/*
...
void sortHeap(DynArr *heap)
{
	...
	int i;
	_buildHeap(heap); /* build proper heap from arbitrary array of values */

	for(i = sizeDynArr(heap) - 1; i > 0; i--)
	{
		swapDynArr(heap, 0, i);  /* swap last element, based on for loop, with first element in array */
		_adjustHeap(heap, i, 0);  /* adjust heap after swap */
	}
